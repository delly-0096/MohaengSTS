...
public class LikesServiceImpl implements ILikesService {
    ...
    /**
     * ✅ 로그인 사용자 memNo 추출
     * - 1) principal에 memNo가 있으면 reflection으로 꺼내보고
     * - 2) 없으면 authentication.getName() = memId 로 MEMBER에서 memNo 조회
     */
    @Override
    public Long resolveMemNo(Authentication authentication) {
        ...
        // 3) principal.getMemUser().getMemNo() 등 다양한 케이스(필요시 확장)
        ...
        if (found == null) throw new IllegalStateException("회원 정보를 찾을 수 없습니다.");
        return found;
    }
