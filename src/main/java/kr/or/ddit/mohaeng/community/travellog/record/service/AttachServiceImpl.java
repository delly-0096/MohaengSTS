...
public class AttachServiceImpl implements IAttachService {
    ...
    public Long saveAndReturnAttachNo(MultipartFile file, long loginMemNo) {
        ...
        if (ext.isBlank()) ext = guessExtFromMime(mimeType);

        String saveName = UUID.randomUUID().toString() + (ext.isBlank() ? "" : "." + ext);

        // 3) DB 경로(스샷 패턴 맞춤: /travellog/cover/yyyy/MM/dd/파일명)
        ...
        // uploadRoot가 "C:/upload/" 라면 -> "C:/upload/travellog/cover/...."
        ...
        try {
            ...
            if (a != 1) throw new RuntimeException("ATTACH_FILE 저장 실패");

            int d = mapper.insertAttachFileDetail(
                    fileNo,
                    attachNo,
                    saveName,       // FILE_NAME
                    ...
                    file.getSize(),
                    dbFilePath,     // FILE_PATH (DB에 저장될 경로)
                    "ATTACH",       // FILE_GB_CD (스샷에 존재)
                    mimeType,       // MIMY_TYPE
                    "Y",            // USE_YN
                    loginMemNo
            );
            if (d != 1) throw new RuntimeException("ATTACH_FILE_DETAIL 저장 실패");

            return attachNo;

        } catch (Exception e) {
            // 실패 시 파일 삭제 시도
            try { Files.deleteIfExists(diskPath); } catch (Exception ignore) {}
            throw new RuntimeException("파일 저장 중 오류: " + e.getMessage(), e);
        }
    }

    private String normalizeRoot(String root) {
        if (root == null || root.isBlank()) return "C:/upload/";
        // 끝에 / 또는 \ 없으면 붙이기
